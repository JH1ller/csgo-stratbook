<template>
  <div class="join-team-form">
    ...
    <form class="join-team-form__form" @submit="joinClicked" novalidate>
      <transition name="fade">
        <span class="join-team-form__error" v-if="formError">{{ formError }}</span>
      </transition>
      <TextInput class="join-team-form__input" v-model="code.value" :field="code" />
      <input type="submit" class="join-team-form__submit" value="Join team" />
    </form>
  </div>
</template>
